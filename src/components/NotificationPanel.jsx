...
import { CancelIcon, NotificationIcon, MailIcon, TickDoubleIcon } from '../lib/icons.jsx'
      ...
      <div className="notif-panel">
        ...
        <div className="notif-panel__email">
          <div className="notif-panel__email-label">
            <MailIcon size={13} />
            <span>{email ? 'Subscribed as' : 'Get notified of new recordings'}</span>
          </div>
          {email ? (
            <div className="notif-panel__subscribed">
              <TickDoubleIcon size={14} />
              <span>{email}</span>
              <button onClick={() => { setEmail(''); localStorage.removeItem('rcf_email') }}>Change</button>
            </div>
          ) : (
