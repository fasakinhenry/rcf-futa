...
import { notifyGalleryApproval, notifyRecordingDrop } from '../lib/email.js';
...
function AddRecordingModal({ categories, onClose, onAdded }) {
  ...
  const handleAudio = (e) => {
    ...
    if (!f.type.startsWith('audio/')) {
      setErrors((e) => ({
        ...e,
        audio: 'Must be an audio file (MP3, WAV, M4A, etc.)',
      }));
      return;
    }
    if (f.size > 200 * 1024 * 1024) {
      setErrors((e) => ({ ...e, audio: 'Max audio size is 200MB' }));
      return;
    }
    setAudioFile(f);
    setErrors((e) => ({ ...e, audio: '' }));
  ...
  const handleCover = (e) => {
    ...
    if (!f.type.startsWith('image/')) {
      setErrors((e) => ({ ...e, cover: 'Must be an image file' }));
      return;
    }
    setCoverFile(f);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      notifyRecordingDrop(rec).catch((err) =>
      ...
      setErrors({
        submit: err.message || 'Upload failed. Check your Cloudinary settings.',
      });
    } finally {
      setSaving(false);
      setStage('');
    ...
    >
      <div className='modal modal--lg' onClick={(e) => e.stopPropagation()}>
        ...
        <p className='modal__subtitle'>
          Subscribers get a new playlist email after you save.
        </p>
        ...
        <div className='modal__body'>
          <div className='form-row'>
            <div className='field'>
              ...
              <input
                ...
                placeholder='e.g. Prayer as a Lifestyle'
                value={form.title}
            ...
            <div className='field'>
              <label>Speaker *</label>
              <input
                className={`input ${errors.speaker ? 'error' : ''}`}
                placeholder='e.g. Bro. Emmanuel Adeyemi'
                value={form.speaker}
          ...
          <div className='field'>
            ...
            <p className='form-hint'>
              Each line = one transcript block. Shown in the expanded player.
            </p>
          </div>
          ...
          <div className='field'>
            ...
            >
              ...
              {audioFile ? (
                <>
                  <span className='file-drop__name'>{audioFile.name}</span>
                  <span className='file-drop__size'>
                ...
                <>
                  ...
                  <span className='file-drop__hint'>
                    Max 200MB · MP3 recommended
                  </span>
                </>
              )}
              {saving && stage === 'audio' && (
                <div className='file-drop__progress'>
          ...
          {saving && stage && (
            <p className='admin-saving-label'>{stageLabel[stage]}</p>
          )}
        </div>
...
export default function Admin() {
  const [loggedIn, setLoggedIn] = useState(
    () => sessionStorage.getItem('rcf_admin') === '1',
  );
  ...
  const login = () => {
    sessionStorage.setItem('rcf_admin', '1');
    setLoggedIn(true);
  };
  const logout = () => {
    sessionStorage.removeItem('rcf_admin');
    setLoggedIn(false);
  };
  ...
  const handleApprove = async (id) => {
    ...
    setStudents((s) =>
      s.map((st) => (st.id === id ? { ...st, approved: true } : st)),
    );
    setAnalytics((a) =>
      a
        ? {
            ...a,
            pendingStudents: a.pendingStudents - 1,
            totalStudents: a.totalStudents + 1,
          }
        : a,
    );

    if (student?.email) {
      notifyGalleryApproval(student).catch((err) =>
  ...
  const handleDeleteUnit = async (id) => {
    if (
      !window.confirm(
        'Delete this unit? Students assigned to it will lose their unit.',
      )
    )
      return;
    await deleteUnit(id);
  ...
  const TABS = [
    ...
    {
      id: 'students',
      label: `Students${pending.length ? ` (${pending.length} pending)` : ''}`,
    },
  ...
  return (
    <main className='admin'>
      <div className='admin__header grid-bg'>
      ...
      <div className='container'>
        <div className='admin__tabs'>
          {TABS.map((t) => (
            <button
              key={t.id}
              className={`admin__tab ${tab === t.id ? 'active' : ''}`}
              onClick={() => setTab(t.id)}
            >
              {t.label}
            </button>
            ...
            {tab === 'analytics' && (
              <div className='admin__section'>
                <div className='admin__stats'>
                  <StatCard
                    label='Students in Gallery'
                    value={analytics?.totalStudents}
                    accent
                  />
                  <StatCard
                    label='Plays This Week'
                    value={analytics?.weeklyPlays}
                  />
                  <StatCard
                  ...
                  <StatCard
                    ...
                    accent={analytics?.pendingStudents > 0}
                  />
                </div>
              </div>
            )}
                      ...
                      <div key={r.id} className='admin__rec-row'>
                        ...
                        <div className='admin__rec-info'>
                          ...
                          <span className='admin__rec-meta'>
                            {r.speaker} · {r.categories?.name} ·{' '}
                            {formatDuration(r.duration_seconds)} ·{' '}
                            {(r.play_count || 0).toLocaleString()} plays ·{' '}
                            {new Date(r.created_at).toLocaleDateString('en-GB')}
                          </span>
                        </div>
                        <span
                          className={`tag ${r.audio_url ? 'tag--green' : 'tag--ghost'}`}
                        >
              ...
              <div className='admin__section'>
                ...
                {pending.length === 0 ? (
                  <div className='admin__empty-small'>
                    <CheckCircleIcon size={18} /> All caught up — no pending
                    approvals
                  </div>
                ) : (
                      ...
                      {units.map((u) => (
                        <div key={u.id} className='admin__item-row'>
                          {editUnit === u.id ? (
                            <EditUnitInline
                              unit={u}
                              onSave={(name) => handleUpdateUnit(u.id, name)}
              ...
              <div className='admin__section'>
                ...
                {subscribers.length === 0 ? (
                  ...
                  <div className='admin__email-grid'>
                    {subscribers.map((s) => (
                      <span key={s.id} className='admin__email-chip'>
                        {s.email}
                      </span>
                    ))}
                  </div>
...
function StudentRow({ student, isPending, onApprove, onDelete }) {
  ...
  return (
    <div
      className={`admin__student-row ${isPending ? 'admin__student-row--pending' : ''}`}
    >
      ...
      <div className='admin__student-info'>
        ...
        <span>
          ...
          {student.units?.name || 'No unit'}
        </span>
        {student.email && <span>{student.email}</span>}
        {student.hobbies && (
          <span className='admin__student-hobbies'>{student.hobbies}</span>
        )}
      </div>
      <span className='admin__student-date'>
        {new Date(student.created_at).toLocaleDateString('en-GB')}
      </span>
      <div className='admin__student-actions'>
        {isPending && (
          <button
            className='admin__icon-btn admin__icon-btn--approve'
            onClick={onApprove}
            title='Approve'
          >
...
function EditUnitInline({ unit, onSave, onCancel }) {
  ...
  return (
    <div className='admin__edit-inline'>
      ...
      <button className='admin__icon-btn' onClick={onCancel}>
        <CancelIcon size={14} />
      </button>
    </div>
  );
